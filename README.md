...
This CLI is the open companion to a TechSkills Academy lead magnet on captioning video locally.
...
│   ├── engine-hf/               # Hyperframes (CSS+GSAP) + 9 presets
│   ├── engine-ass/              # ffmpeg+libass + presets/
│   │   └── src/presets/         # only `clean-white.ts` ships in git;
│   │                            # basic + premium are gitignored, repopulated
│   │                            # at build time by scripts/install-pack.sh
│   └── cli/                     # bin entry, dispatcher
├── packs/                       # GITIGNORED — paid pack source
│   ├── basic/                   # 4 presets (47 PLN)
│   └── premium/                 # 20 presets (97 PLN)
├── scripts/
...
get a ready-to-drop preset file. Move it into `packs/premium/`, run
...
- **TechSkills Academy lead magnet** (Polish) — the guide on captioning video locally that
  this CLI accompanies.
